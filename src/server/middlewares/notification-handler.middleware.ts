...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order_placed.create': { roles: ['owner', 'administrator', 'manager'], key: 'order', tenantPath: [], userPath: [] },
  ...
  'item_added.create': { roles: ['owner', 'administrator', 'manager'], key: 'inventory', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['owner'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['owner', 'administrator', 'manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.agency.findFirst({
