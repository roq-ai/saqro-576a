...
import { getSmoServiceById } from 'apiSdk/smo-services';
import { SmoServiceInterface } from 'interfaces/smo-service';

function SmoServiceViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<SmoServiceInterface>(
    () => (id ? `/smo-services/${id}` : null),
    () =>
      getSmoServiceById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('smo_service', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/smo-services/edit/${id}`} passHref legacyBehavior>
                    <Button
