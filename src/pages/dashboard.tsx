...
import { getSmoServices } from 'apiSdk/smo-services';
import { SmoServiceInterface } from 'interfaces/smo-service';
import { getSeoServices } from 'apiSdk/seo-services';
import { SeoServiceInterface } from 'interfaces/seo-service';
import { getEmailMarketingServices } from 'apiSdk/email-marketing-services';
import { EmailMarketingServiceInterface } from 'interfaces/email-marketing-service';

import { EmailMarketingServiceListPage as ShowCaseList } from 'pages/email-marketing-services';

function DashboardPage() {
  const { session } = useSession();

  const smoServiceStat = useSWR<PaginatedInterface<SmoServiceInterface>>(
    () => `/smo-services`,
    () => getSmoServices({ limit: 1, offset: 0 }),
  );

  const seoServiceStat = useSWR<PaginatedInterface<SeoServiceInterface>>(
    () => `/seo-services`,
    () => getSeoServices({ limit: 1, offset: 0 }),
  );

  const emailMarketingServiceStat = useSWR<PaginatedInterface<EmailMarketingServiceInterface>>(
    () => `/email-marketing-services`,
    () => getEmailMarketingServices({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Smo Services',
      result: smoServiceStat?.data?.totalCount,
      isLoading: smoServiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Seo Services',
      result: seoServiceStat?.data?.totalCount,
      isLoading: seoServiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Email Marketing Services',
      result: emailMarketingServiceStat?.data?.totalCount,
      isLoading: emailMarketingServiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Email Marketing Services
                </Text>
                <NextLink href={`email-marketing-services`} passHref legacyBehavior>
                  <Button
                  ...
                  >
                    See all Email Marketing Services
                  </Button>
                </NextLink>
              </Flex>
